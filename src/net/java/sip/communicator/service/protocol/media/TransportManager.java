        ...
        try {
            ...
            synchronized (connector) {
                ...
                connector.getDataSocket().send(new DatagramPacket(
                        "0".getBytes(), 1, target.getDataAddress().getAddress(),
                        target.getDataAddress().getPort()));

                /* control port (RTCP) */
                connector.getControlSocket().send(new DatagramPacket(
                        "0".getBytes(), 1, target.getControlAddress().getAddress(),
                        target.getControlAddress().getPort()));
            }
        }
        catch (Exception e) {
            logger.error("Error cannot send to remote peer: " + target.getDataAddress().getHostName() + ":" + target.getDataAddress().getPort(), e);
        }
    }
